...
class Value(object):
    ...
    A camera might capture an :class:`Image`, an IMU will record :class:`ImuValue` or the motion capture system 
    :class:`GroundTruth`.
     
    All these values have some common properties in the StereoTUM:
    1) They are single, time-discrete values (see :attr:`stamp`)
    2) They are related to a certain reference frame (see :attr:`reference`)
    3) They all have a transformation from its reference frame towards that of "cam1" (see :attr:`transform`)
    ...
    * or another value, whose :attr:`reference` property is used to determine the transform
    ...
    def _transform(self):
        r"""
        The transformation from "cam1" to this value`s :attr:`reference` as 4x4 numpy.ndarray homogenous matrix 
        """
